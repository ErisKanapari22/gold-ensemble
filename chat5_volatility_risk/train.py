# train.py
# Trains the VolatilityRiskModel and saves the best checkpoint on DIRECTIONAL F1.
# Run: python train.py
...
def train():
    ...
    weight_str = "  ".join(
        f"{n}={class_weights[i]:.1f}" for i, n in enumerate(config.CLASS_NAMES)
    )
    print(f"Loss class weights: {weight_str}")

    model     = VolatilityRiskModel().to(device)
    ...
    for epoch in range(1, config.EPOCHS + 1):
        ...
        # Per-class recall — watch for class collapse (recall → 0 on DIRECTIONAL)
        conf = torch.zeros(config.N_CLASSES, config.N_CLASSES, dtype=torch.long)
        for t, p in zip(all_true, all_pred):
            conf[t][p] += 1

        for i, name in enumerate(config.CLASS_NAMES):
            tp    = conf[i, i].item()
            total = conf[i].sum().item()
            recall = tp / total if total > 0 else 0.0
            print(f"    {name} recall: {recall:.3f}  ({tp}/{total})")

        # DIRECTIONAL F1 — checkpoint metric
        dp      = conf[0, 0].item()
        dp_pred = conf[:, 0].sum().item()
        dp_true = conf[0].sum().item()
