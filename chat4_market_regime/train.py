# train.py
# Trains the MarketRegimeModel and saves the best checkpoint based on
# DIRECTIONAL F1. Run: python train.py
#
# IMPORTANT: run diagnose.py first and confirm config.USE_HIDDEN_LAYER
# matches its recommendation before training.
...
def train():
    ...
    print(f"Architecture: {'MLP (hidden layer)' if config.USE_HIDDEN_LAYER else 'Logistic regression (BatchNorm + Linear)'}")

    # Data (WeightedRandomSampler already applied to train_loader inside get_dataloaders)
    train_loader, val_loader = get_dataloaders()

    # Class weights as secondary safeguard on top of the sampler
    ...
    print(f"Class weights (loss): {weight_str}")

    # Model, loss, optimizer, scheduler
    model = MarketRegimeModel().to(device)
    ...
    print(
        "\nNote for Chat 0 / Chat 7: report this F1 honestly as the Chat 4 signal "
        "strength when weighting the ensemble — do not artificially inflate it."
    )
