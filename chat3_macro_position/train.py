# chat3_macro_position/train.py
# Trains the MacroPositionModel and saves the best checkpoint based on
...
def train():
    ...
    for epoch in range(1, config.EPOCHS + 1):
        ...
        # ── DIRECTIONAL F1 ─────────────────────────────────────────────────────
        tp_d   = conf_matrix[0, 0].item()
        pred_d = conf_matrix[:, 0].sum().item()
        true_d = conf_matrix[0].sum().item()
        prec   = tp_d / pred_d if pred_d > 0 else 0.0
        rec    = tp_d / true_d if true_d > 0 else 0.0
        dir_f1 = 2 * prec * rec / (prec + rec) if (prec + rec) > 0 else 0.0
        print(f"    DIRECTIONAL F1: {dir_f1:.3f}  (prec={prec:.3f}  rec={rec:.3f})")

        # ── Checkpoint on best DIRECTIONAL F1 ─────────────────────────────────
